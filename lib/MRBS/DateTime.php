...
class DateTime extends \DateTime
{
  ...
  public function isHoliday() : bool
  {
    ...
    // Format the date to ISO format for the query
    $iso_date = $this->format('Y-m-d');

    // Use db() to get the database connection and check if this date is a holiday
    $query = "SELECT COUNT(*) FROM mrbs_holidays WHERE holiday_date = ?";
    $count = db()->query1($query, [$iso_date]);

    // Return true if there's a holiday on this date, false otherwise
    return $count > 0;	
