...
use MRBS\Form\ElementA; //added only
use MRBS\Form\ElementText; //added only
use MRBS\Form\ElementRaw; //added only
...
function get_field_custom(string $key, bool $disabled=false)
{
  ...
  //echo '<pre>',var_dump("Entering get_field_custom() with key: $key,"),'</pre>';
  ...
  else
  {
	...
	/*------start of edit (Contact Requirements)------*/
    $field = get_field_entry_input($params);
	
    if ($key == 'contact_requirements')
    {
      // Create a <div> that will contain the original control and the new text
      $div = new ElementDiv();
      // Create a <span> that will contain the new text (and link)
      $a = new ElementA();
      $a->setAttribute('href', '#');
      $a->setAttribute('onclick', 'openNModal(); return false;');
      $a->setText("Notation");
      $span = new ElementSpan();
      //$span->setText("Read ", true);
      $span->addElement($a);
      // Get and remove the control
      $control = $field->getControl();
      $field->removeControl();
      // Put everything in the <div> and then into the field
      $div->addElement($control)->addElement($span);
      $field->addElement($div);
    }
    return $field;
  /*------end of edit (Contact Requirements)------*/
  ...
  /*------start of edit (Terms and Conditions)------*/
   if ($key == 'terms_and_conditions'){
		if (is_book_admin()) {
			$field = new ElementInputHidden(VAR_PREFIX . 'terms_and_conditions', '1'); // Auto-checked for admins
		} else {
			$div = new ElementDiv();
			$a = new ElementA();
			$a->setAttribute('href', '#');
  ...
  /*-------end of edit (Terms and Conditions)-------*/
  return $field;
}
...
<div id="terms-modal" class="modal">
  <div class="modal-content">
    ...
    <h4 class="terms" style="color: green">Terms and Conditions</h4>
    <p class="termcontent">
      By reserving a meeting/event room, you agree that the booking is subject to approval.
      The room must be used for the specified time, and logistical requirements must be included when booking.
      Cancellation or deletion of bookings should be made at least 2 days prior to the event; repeated no-shows
      may lead to future booking restrictions. You are responsible for any damage caused and must adhere to venue
      rules, including capacity limits and safety guidelines. All personal data that will be collected are handled
...
echo '
<div id="notation-modal" class="nmodal">
  <div class="nmodal-content">
    <span class="nclose-btn">&times;</span>
    <h4 class="notation" style="color: green">Notation</h4>
	<table border=1>
		<tr>
			<td><b>Sound System</b></td>
			<td>Can be provided by the PGLU. However, requesting party can outsource 
			    their preferred provider.</td>
		</tr>
		<tr>
			<td><b>LED Wall</b></td>
			<td>PGLU does not have any available LED Wall at the moment. Requesting party are  
	            advised to outsource their provider subject to coordination with PEO for  
			    the computation of power and proper tapping of electricity. </td>
		</tr>
		<tr>
			<td><b>Chairs</b></td>
			<td>PGLU can provide monoblock chairs at a maximum of 1,100. Chairs beyond that number 
			    shall be outsourced by the requesting party. Special chairs (Tiffany, sofa sets or 
				the like) are to be outsourced by the requesting party. </td>
		</tr>
		<tr>
			<td><b>Tables</b></td>
			<td>PGLU can provide monoblock tables (6 seater rectangular) at a maximum of 40. Table 
			    requirement beyond the maximum shall be outsourced by the requesting party. Special 
				tables (Round, banquet, or the like) are to be outsourced by the requesting party.</td>
		</tr>
		<tr>
			<td><b>Table and chair cover</b></td>
			<td>PGLU does not have any available table and chair covers at the moment. 
			    Requesting party are advised to outsource their provider</td>
		</tr>
		<tr>
			<td><b>Gas</b></td>
			<td>Requesting party shall allocate 100 Liters per hour of gasoline for the 
			    standby Generator set per day.</td>
		</tr>
		<tr>
			<td><b>Other Equipment (water dispenser, projector, extension wires, TV, or the like not 
			    listed in this table)</b></td>
			<td>Requesting party are advised to coordinate with other offices or with their service 
			    providers.</td>
		</tr>
		<tr>
			<td><b>Catering Services</b></td>
			<td>Requesting parties are allowed to bring their own catering service provider provided 
			    that they shall ensure the cleanliness and orderliness before, during 
				and after event.</td>
		</tr>
		<tr>
			<td><b>Internet Connection</b></td>
			<td>Can be provided by the PGLU upon request.</td>
		</tr>
		<tr>
			<td><b>Ingress</b></td>
			<td>Requesting parties are advised to include or block off the date and time of 
			    ingress in the pre-booking system.</td>
		</tr>
		<tr>
			<td><b>Egress</b></td>
			<td>Requesting parties shall ensure that all materials, equipment and other items used 
			    during their activities shall be brought out from LUCC within the day to prepare 
				the same for the next requesting party.</td>
		</tr>
	</table>
  </div>
</div>
